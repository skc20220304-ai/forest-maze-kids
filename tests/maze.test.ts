import {describe,expect,it} from 'vitest';import {stages} from '../src/stages/stages';import {validateDifficultyProgression,validateStage} from '../src/domain/maze';import {initialState,move} from '../src/domain/reducer';
describe('maze safety',()=>{
  it('validates every stage and keeps IDs unique',()=>{
    expect(stages.map(stage=>stage.id)).toEqual(Array.from({length:100},(_,index)=>index+1));
    expect(new Set(stages.map(stage=>stage.layout.join('|'))).size).toBe(100);
    ...
    expect(validateDifficultyProgression(stages)).toBe(true);
