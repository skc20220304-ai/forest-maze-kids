...
export async function syncProgress(localStage:number){if(!supabase)return{stage:localStage,synced:false};const remote=await loadCloudProgress();const stage=Math.max(localStage,remote??1);const synced=await saveCloudProgress(stage);return{stage,synced}}
